...
jssy.init = options => {
    ...
    // Asserts
    assert(jssy.options.db, "Invalid db config");

    // Init db
    jssy.db = lowdb(jssy.options.db);

    // Initialize users
    var users = jssy.db.get("users").value();
    if (!users) {
        jssy.db.set("users", [{
            username: "admin",
    ...
    jssy.pre("init", express.static(path.join(__dirname, "../build")));
...
jssy.start = cb => {
    // Asserts
    assert(jssy.options, "Invalid options");
